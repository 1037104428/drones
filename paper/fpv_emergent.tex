...
每机每拍只能感知圈内物体，8 是记忆上限不是全知。假想敌用光电/寻的半径 $D_{\mathrm{tgt}}=50\,\mathrm{m}$（无线电压制下不能靠数据链把远点送进座舱）；友军用目视/近距通视 $D_{\mathrm{fr}}=100\,\mathrm{m}$（编队里 40 m 排距、80 m 邻轨的机体仍看得见，但看不到更远的电台位置）。圈外目标与友机不入库。开火要求目标在 $D_{\mathrm{tgt}}$ 内。编队默认为两排并排。
...
\begin{figure}[H]
  ...
  \caption{30 轮平均存活时间分布（越低越好）。贪心最强，涌现规则其次，Transformer 最弱。}
  \label{fig:surv}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{30 轮中和数分布。一次性上限 12；贪心约 11.4，规则约 10.1，Transformer 约 5.6。}
  \label{fig:kill}
\end{figure}
...
\begin{enumerate}
  \item \textbf{效能。} 并排、圈内 8 槽上限下，共享 Transformer（逐步 RL）与贪心同为中和 10.23、存活 13.99\,s；近邻规则 9.80、14.39\,s。12 机共用一套权重。
...
\end{verbatim}
仓库中 \texttt{results/*.csv} 为本文表~\ref{tab:res}--\ref{tab:pval} 的评估记录；\texttt{models/transformer.json} 为 200 轮逐步 REINFORCE 后的共享权重。完整 SQLite 可由上述命令在本地生成。同一种子重复评估时，中和结果与 \texttt{compute\_ops} 逐行一致。英文稿见 \texttt{fpv\_emergent\_en.pdf}。
