...
\texttt{nearest\_in\_range} 在探测圈内、且未被他机锁定的目标中取欧氏距离最小者开火（并列取更小 id）。不看友机距离。每步约 9 次操作。它是``不做神经网络、也不做去冲突''的下限对照。
...
贪心与共享 Transformer 在 30 轮上逐轮相同（网络学会了就近开火）。近邻规则因 $D_{\mathrm{fr}}=100\,\mathrm{m}$ 总能看见 40 m 处的后排，开火更挑剔，中和少 0.43、存活高 0.40\,s（$p=9.7\times 10^{-9}$）。删失基线 $T_{\mathrm{end}}=21.6\,\mathrm{s}$。训练中若不对开火给即时奖励，REINFORCE 会在约第 90 轮塌成全弃权（$killed=0$）；加上开火 shaping 与熵正则后稳定在约 10 杀。
...
相对并排格子，高斯进入使贪心/网络场均中和从 10.23 升到 11.30：机群在圆心附近更密，50 m 光电重叠更好。规则仍显著更保守（少约 1.17 杀，$p\sim 10^{-8}$），因为 $D_{\mathrm{fr}}=100\,\mathrm{m}$ 下近距友机更容易被看见。Transformer 再次与贪心逐轮相同。结论对进场几何稳健：网络没有多打掉目标。
...
\begin{enumerate}
  \item \textbf{效能。} 并排下共享 Transformer 与贪心同为中和 10.23、存活 13.99\,s；近邻规则 9.80、14.39\,s。高斯进入下三者为 11.30 / 10.13 / 11.30 杀。12 机共用一套权重。
