...
每机每拍只能感知圈内物体，8 是记忆上限不是全知。假想敌用光电/寻的半径 $D_{\mathrm{tgt}}=50\,\mathrm{m}$（无线电压制下不能靠数据链把远点送进座舱）；友军用目视/近距通视 $D_{\mathrm{fr}}=100\,\mathrm{m}$（编队里 40 m 排距、80 m 邻轨的机体仍看得见，但看不到更远的电台位置）。圈外目标与友机不入库。开火要求目标在 $D_{\mathrm{tgt}}$ 内。编队默认为两排并排。另可选高斯进入：机群从正南以二维正态散布飞入（$\sigma_x=70\,\mathrm{m}$，$\sigma_y=30\,\mathrm{m}$，机间距至少 8 m），模拟不整齐的实际进场。
...
相对并排格子，高斯进入使贪心/无通信贪心/网络场均中和从 10.23 升到 11.30：机群在圆心附近更密，50 m 光电重叠更好。无通信贪心与 \texttt{nearest\_in\_range} 在两种进场下均逐轮相同。规则仍显著更保守（少约 1.17 杀，$p\sim 10^{-8}$），因为 $D_{\mathrm{fr}}=100\,\mathrm{m}$ 下近距友机更容易被看见。Transformer 再次与贪心逐轮相同。结论对进场几何稳健：零通信贪心已经是该传感器模型下的杀伤上限，网络没有多打掉目标。
...
\end{verbatim}
仓库中 \texttt{results/*.csv} 为并排评估（表~\ref{tab:res}），\texttt{results/gaussian/*.csv} 为高斯进入（表~\ref{tab:res-g}）。\texttt{models/transformer.json} 与 \texttt{models/transformer\_gaussian.json} 分别为两套进场上训练的共享权重。高斯进入复现：
\begin{verbatim}
cargo run --release -- experiment --ingress gaussian \
  --train-rounds 200 --eval-rounds 30 --seed 20260823
\end{verbatim}
